...
def move_to_points(points, s, video, K, dist_cv):
    ...
    while idx < len(points):
        ...
        #print(points)
        print(points[idx])
        if ((points[idx][0] == 15) and (points[idx][1]==35)):
            print(points[idx])
            isNonHelperWaypoint = True
        else:
            isNonHelperWaypoint = False
        if (idx>0):
            if ((points[idx][0] == 15) and (points[idx][1]==35)):
                CORRECT_FORWARD_PLS = True
            else:
                CORRECT_FORWARD_PLS = False
        ...
        if stable_counter >= STABLE_FRAMES and not backing_up:
            ...
            aligning = False
            if (not isNonHelperWaypoint):
                aligning = True
            stable_counter = 0
            continue
        print(isNonHelperWaypoint)
        if (isNonHelperWaypoint):
            # ------------------------------------------------------------
            # BACKUP STATE (pulse-based)
            # ------------------------------------------------------------
            if backing_up:
                moved = np.linalg.norm(current - backup_start)
                print(f"[BACKUP] {moved:.2f}/{BACKUP_DIST}")

                if np.linalg.norm(current - points[idx]) < 4.5:
                    pulse_move_linear(s, heading_deg=180,
                                    power=BACKUP_POWER,
                                    ms=BACKUP_PULSE_MS)
                    continue
                else:
        ...
        # ------------------------------------------------------------
        if forward_after_align and CORRECT_FORWARD_PLS:
