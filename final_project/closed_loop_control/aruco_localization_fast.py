...
def get_pose(frame_small):
    ...
    # -------------------------------------------------
    # Detect markers on the DOWNSAMPLED frame
    # -------------------------------------------------
    corners, ids, _ = cv2.aruco.detectMarkers(frame_small, ARUCO_DICT, parameters=PARAMS)
    ...
    # -------------------------------------------------
    # UPSCALE ALL CORNERS BACK TO FULL-RES PIXELS
    # -------------------------------------------------
    up_corners = []
    for c in corners:
        c_up = c.copy()
        c_up[0] *= INV_SCALE
        up_corners.append(c_up)

    corners = up_corners

    # -------------------------------------------------
    # Build center dictionary
    # -------------------------------------------------
    centers = {i: np.mean(c[0], axis=0) for i, c in zip(ids, corners)}

    # -------------------------------------------------
    # Update / reuse homography
    # -------------------------------------------------
    if all(i in centers for i in CORNER_IDS):
        last_centers = centers
        H = compute_homography(centers)
    ...
    # -------------------------------------------------
    # Robot center → world point
    # -------------------------------------------------
    rc = centers[ROBOT_ID]
    pt_img = np.array([[rc[0], rc[1], 1.0]]).T    # ALWAYS 3×1 now
    ...
    # -------------------------------------------------
    # Orientation (also using upscaled corners)
    # -------------------------------------------------
    robot_corners = None
    for i, c in zip(ids, corners):
        if i == ROBOT_ID:
    ...
    # Convert to homogenous and project
    p0w = H @ np.array([[c0[0], c0[1], 1.0]]).T
    p3w = H @ np.array([[c3[0], c3[1], 1.0]]).T
    ...
    # -------------------------------------------------
    # Return pose + the small frame for debug
    # -------------------------------------------------
    return (x, y, theta_world), frame_small
